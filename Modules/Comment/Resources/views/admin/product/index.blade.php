...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('admin.product') }}">فروشگاه</a></li>
            ...
            <div class="pb-0 mb-2">
                <div class="d-flex justify-content-between">
                    <div class="d-flex gap-2">
                        <h4 class="card-title mg-b-0">نظرات مجصولات</h4>
                                        ...
                                        <div class="filter-items  d-flex justify-content-start align-items-center gap-1">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-filter rounded shadow tx-12" id="myDropdown2">
                                                ...
                                                @foreach($products as $product)
                                                    <a href="{{ route('admin.comment', 'commentable_id=' . $product->id) }}" class="dropdown-item @if((str_contains(request()->getUri(), '?commentable_id=' . $product->id))) bg-primary-transparent @endif" data-bs-placement="left" data-bs-toggle="tooltip" title="{{ $product->title }}">{{ Str::limit($product->title, 25) }}</a>
                                                @endforeach
                                            </div>
                                        ...
                                        @foreach($comments as $comment)
                                            <tr>
                                                <td>
                                                    ...
                                                    <small class="tx-10"><a href="{{ route('admin.product.show', $comment->commentable) }}">{{ $comment->commentable->title ?? '-' }}</a></small>
                                                </td>
                                                <td><a href="{{ route('admin.product.comment.show', $comment) }}"><small>{{ $comment->parent->body ?? '-' }}</small></a>
                                                </td>
                                                <td><a href="#">{{$comment->author->fullname ?? $comment->author->name ?? '-' }}</a></td>
                                                ...
                                                <td class="d-flex justify-content-start">
                                                    ...
                                                    <a href="{{ route('admin.product.comment.show', $comment) }}" class="btn-sm"><i class="fe fe-eye"></i> نمایش</a>
                                                    <form action="{{ route('admin.comment.destroy', $comment) }}" method="product">
                                                        @csrf @method('delete')
                                                ...
                                                <div class="modal-dialog" style="max-width: 35%">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-header">
                                                            ...
                                                            <p><a href="{{ route('admin.product.show', $comment->commentable) }}" class="btn btn-link btn-sm">{{ $comment->commentable->title }}</a></p>
                                                        ...
                                                        <div class="card card-body pd-20 pd-md-40 m-1 border-0 shadow-none">
                                                            ...
                                                            <form action="{{ route('admin.comment.answer', $comment) }}" method="post">
                                                                ...
                                                                <div class="form-group mb-4">
                                                                    <div class="main-form-group mb-4">
