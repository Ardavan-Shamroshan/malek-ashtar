...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">ساخت بنر</li>
        </ol>
    </nav>
    <!-- breadcrumb -->
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="pb-0 mb-2">
                <div class="d-flex justify-content-between">
                    <div class="d-flex gap-2">
                        <h4 class="card-title mg-b-0">بنر ها</h4>
                        <p class="tx-12 tx-gray-500 mb-2">ساخت بنر.
            ...
            {{-- form --}}
            <form action="{{ route('admin.banner.store') }}" id="form" method="post" enctype="multipart/form-data">
                @csrf
                                        ...
                                        <div class="SumoSelect sumo_somename" tabindex="0" role="button" aria-expanded="false">
                                            <select name="position" id="position" class="form-control SlectBox SumoUnder @error('position') border-danger @enderror" tabindex="-1">
                                                ...
                                                @foreach($positions as $key => $value)
                                                    <option value="{{ $key }}" @selected(old('position') == $key)>{{ $value }}</option>
                                                @endforeach
                                            </select>
                                            @error('position')
